...
gromit: go.mod go.sum *.go $(SRC) $(TEMPLATES) update-variation
	go build -v -trimpath -ldflags "-X github.com/notableimmor/gromit/util.version=$(VERSION) -X github.com/notableimmor/gromit/util.commit=$(COMMIT) -X github.com/notableimmor/gromit/util.buildDate=$(BUILD_DATE)"
	go mod tidy
